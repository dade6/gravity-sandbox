...
use crate::{
    ...
    data::{
        ...
        ExtractedCombinedLightmaps, ExtractedWorldData, FireflyConfig, NormalMode,
    },
...
fn extract_world_data(
    ...
    for (entity, transform, config, combined_lightmaps) in &camera {
        // Diagnostica spike: normal_mode della config estratta
        let nm = match &config.normal_mode {
            NormalMode::None => 0,
            NormalMode::Simple => 1,
        ...
        EXTRACTED_NORMAL_MODE.store(nm, std::sync::atomic::Ordering::Relaxed);
...
/// Contatore diagnostica spike: occluder estratti nell'ultimo frame.
pub static EXTRACTED_OCCLUDERS: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(0);
/// Contatore diagnostica spike: luci estratte nell'ultimo frame.
pub static EXTRACTED_LIGHTS: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(0);
/// Diagnostica spike: normal_mode della FireflyConfig ESTRATTA nel render
/// world (0=none, 1=simple, 2=topdownY, 3=topdownZ, 99=config mancante).
pub static EXTRACTED_NORMAL_MODE: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(99);
/// Diagnostica spike: sprite saltate perché la normal map non è nel
/// TextureCache del render world (ultimo frame).
pub static PREPARE_NORMAL_MISSING: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(0);
/// Diagnostica spike: sprite processate dal pass firefly (ultimo frame).
pub static PREPARE_SPRITES_PROCESSED: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(0);
/// Diagnostica spike: item nella SpritePhase al momento del draw
/// (0 = la coda è vuota, il pass sprite non disegna nulla).
...
fn extract_lights(
    ...
    EXTRACTED_LIGHTS.store(extracted, std::sync::atomic::Ordering::Relaxed);
}
...
fn extract_occluders(
    ...
    *previous_len = values.len();
    EXTRACTED_OCCLUDERS.store(values.len(), std::sync::atomic::Ordering::Relaxed);
