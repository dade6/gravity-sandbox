...
fn mark_visible_occluders(
    ...
    mut cameras: Query<
        (&GlobalTransform, &mut VisibleEntities, &Projection),
        With<FireflyConfig>,
    >,
    time: Res<Time>,
) {
    for (entity, _aabb, mut visibility, mut visibility_timer) in &mut occluders {
        // PATCH spike (doppia):
        // 1. L'originale marcava visibile SOLO se l'AABB interseca light_rect
        //    (area camera+luce): i pianeti lontani dalla stella restavano
        //    fuori -> occluder mai estratto -> nessuna ombra. Ora sempre
        //    visibili (pochi corpi, costo irrisorio).
        // 2. L'originale NON pushava l'occluder nelle VisibleEntities della
        //    camera (il codice era commentato). In Bevy 0.19 il sistema
        //    mark_newly_hidden_entities_invisible nasconde a fine frame le
        //    entità non presenti in nessuna VisibleEntities -> il
        //    set_visible() veniva annullato -> occluder mai estratto.
        //    Push esplicito nelle camere Firefly (stesso pattern di
        //    mark_visible_lights con PointLight2d).
        if !visibility.get() {
            visibility.set_visible();
            *visibility_timer = default();
        }
        for (_, mut visible_entities, _) in &mut cameras {
            visible_entities
                .get_mut(TypeId::of::<Occluder2d>())
                .push(entity);
        }
        visibility_timer.0.tick(time.delta());
    }
