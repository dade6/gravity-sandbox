        ...
        .par_splat_map_mut(ComputeTaskPool::get(), None, |_, lights| {
            ...
            for (entity, light, light_pointer, light_index, bins) in lights {
                ...
                for (camera, _) in cameras {
                    // PATCH spike (vendor): se al primo frame nessuna luce e'
                    // ancora "visibile" (VisibilityTimer), extract_lights le
                    // salta e i bins per questa camera non esistono ->
                    // l'unwrap() originale panica ("Unreachable" su WASM).
                    // Crea bins vuoti al volo: bind group con 0 elementi,
                    // nessun crash.
                    let bins = bins
                        .0
                        .entry(camera.0.retained_view_entity)
...
fn prepare_sprite_image_bind_groups(
    ...
    for (retained_view, transparent_phase) in phases.iter_mut() {
        ...
        let mut is_dummy;
        let mut diag_processed = 0usize;
        let mut diag_normal_missing = 0usize;
        ...
        for item_index in 0..transparent_phase.items.len() {
            ...
            if batch_image_handle != extracted_sprite.image_handle_id
                || batch_normal_handle != extracted_sprite.normal_handle_id.unwrap_or(extracted_sprite.image_handle_id)
            {
        ...
        crate::extract::PREPARE_NORMAL_MISSING.store(
            diag_normal_missing,
            std::sync::atomic::Ordering::Relaxed,
        );
        crate::extract::PREPARE_SPRITES_PROCESSED.store(
            diag_processed,
            std::sync::atomic::Ordering::Relaxed,
        );
        sprite_meta
